...
- **Update visitor count** (`update-visitor-count.yml`, every 30 min) — fetches just the last 3 days of stats from GoatCounter (small, cheap request) and merges them into a local day-by-day cache (`.github/data/visitor-count.json`). The lifetime total patched into `index.html`'s footer is derived by summing that cache rather than asked of the API directly (GoatCounter's `total` field only covers the requested date range), and the per-day breakdown is patched into `visitors.html`'s chart data.
